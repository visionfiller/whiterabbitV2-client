                        ...
                        newArray.map((wine) => {
                            return (<div className="" key={wine.id}>
                                <button onClick={(event) => HandleCardClick(event, wine)} key={wine.id} id={wine.id} className="badge badge-sm  bg-transparent border-none  font-semibold p-8 my-8 w-full transform hover:scale-125  transition ease-out duration-300">
                                    {wine.varietal.wineTypeId === 2 ? <><div className="bg-transparent inline-block  absolute  mx-auto text-lg md:text-xl text-secondary">{wine.region?.location} {wine.varietal?.name}</div><img src="https://www.onlygfx.com/wp-content/uploads/2017/04/yellow-paint-brush-stroke-9-300x122.png" className="w-full md:w-full" /></>
